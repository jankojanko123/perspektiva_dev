...
class _FlutterFlowGoogleMapState extends State<FlutterFlowGoogleMap> {
  ...
  void initializeMarkerBitmap() {
    ...
    void initState() {
      ...
      circleCenter =
          initialPosition; // Set initial circle center to the initial map position
...
/*
  LatLng circleCenter =
      _FlutterFlowGoogleMapState().currentMapCenter; /* ISSUE */
*/
  late LatLng circleCenter; // Declare it without initializing
  //LatLng circleCentera =  initialPosition;

  //LatLng(37.7749, -122.4194); // Example: San Francisco
  double circleRadius = 50.0; // Example: 5 km
...
/* restrict camera movement
        onCameraMove: (position) {
          LatLng restrictedPosition = restrictMovement(position.target);
          if (restrictedPosition != position.target) {
            currentMapCenter = restrictedPosition;
            _controller.future.then((mapController) {
              mapController.animateCamera(
                CameraUpdate.newLatLng(restrictedPosition),
              );
            });
          } else {
